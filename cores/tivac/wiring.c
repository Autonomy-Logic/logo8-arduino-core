...
{
	// Read milliseconds and the SysTick counter consistently: if the SysTick
	// interrupt fires between the two reads it both increments milliseconds and
	// reloads the counter, which otherwise yields a value ~1 ms in the past
	// (micros() going backwards). Retry until milliseconds is stable so the
	// result is monotonic.
	unsigned long m, val;
	do {
		m = milliseconds;
		val = MAP_SysTickValueGet();
	} while (m != milliseconds);
	return (m * 1000) + ( ((F_CPU / SYSTICKHZ) - val) / (F_CPU/1000000));
}
